"""
Standalone Improved Telegram Bot for Printing Business Platform
...
# Ensure we're using the right telegram imports
try:
...
✅ ቦቱ በጥሩ ሁኔታ እየሰራ ነው
🤖 ስሪት: 2.1 የተሻሻለ እና የተስተካከለ
⏰ አሁኑን ጊዜ: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
🏪 ቢዝነስ: {BUSINESS_NAME}
🔧 Python-telegram-bot ስሪት: {telegram.__version__}
...
📞 <i>ለዝርዝር ዋጋዎች እና ልዩ መስፈርቶች ያነጋግሩን!</i>
    ...
    async def handle_callback_query(self, update: Update, context) -> None:
        ...
        if query.data.startswith("set_admin_"):
            ...
            service = query.data.replace("order_", "").replace("_", " ").title()
            service_amharic = {
                "Business Cards": "የንግድ ካርዶች",
                "Flyers": "ፍላየሮች", 
                "Banners": "ባነሮች",
                "Booklets": "መጽሄቶች",
                "Stickers": "ስቲከሮች",
                "Custom": "ልዩ"
            }.get(service.split()[0], service)
            
            await query.edit_message_text(
                f"🛒 <b>{service_amharic} ትዕዛዝ</b>\n\n"
                f"ጥሩ ምርጫ! ለ{service_amharic} ትዕዛዝዎ እባክዎን:\n\n"
                f"1️⃣ መስፈርቶችዎን ይንገሩን\n"
                f"2️⃣ የሚፈልጉትን ብዛት ይጥቀሱ\n"
                f"3️⃣ የሚመችዎትን ቀን ይምረጡ\n\n"
                f"📱 ያነጋግሩን:\n"
                f"📧 {BUSINESS_EMAIL}\n"
                f"📞 {BUSINESS_PHONE}\n\n"
                f"ወይም 💬 ያነጋግሩን በመጠቀም መልእክት ይላኩ!",
                parse_mode='HTML'
            )
            ...
            if contact_type == "email":
                await query.edit_message_text(
                    ...
                    f"ኢሜይል ይላኩልን:\n"
                    f"📧 {BUSINESS_EMAIL}\n\n"
                    f"አብዛኛውን ጊዜ በ24 ሰአት ውስጥ መልስ እንሰጣለን!",
                    parse_mode='HTML'
                )
                ...
                await query.edit_message_text(
                    ...
                    f"ይደውሉልን:\n"
                    f"📞 {BUSINESS_PHONE}\n\n"
                    f"የስራ ሰአት: 24/7 ነው። በማንኛውንም ጊዜ ይደውሉ!",
                    parse_mode='HTML'
                )
            elif contact_type == "message":
                await query.edit_message_text(
                    f"💬 <b>ፈጣን መልእክት</b>\n\n"
                    f"መልእክትዎን ብቻ ይተይቡ፣ ወደ ቡድናችን እንዲደርስ አደርጋለሁ!\n\n"
                    f"ይህንን ያካትቱ:\n"
                    f"• ስምዎን\n"
                    f"• የመገኛ ዝርዝርዎን\n"
                    f"• መልእክትዎን/መስፈርቶችዎን",
    ...
    async def handle_schedule_callback(self, update: Update, context):
        ...
        await update.callback_query.edit_message_text(
            ...
            "💬 You can send this info as a message or contact us directly!",
            parse_mode='HTML'
        )
...
💬 Telegram: {BUSINESS_USERNAME}
                ...
                await update.message.reply_text(
                    "🤔 ያንን አልተረዳሁትም።\n\n"
                    "እባክዎን ከታች ያሉትን የሜኑ ቁልፎች ይጠቀሙ ወይም /help ይተይቡ።",
                    reply_markup=ReplyKeyboardMarkup(
                        [
                            ['🏪 አገልግሎቶች', '🛒 ትዕዛዝ ይስጡ'],
                            ['📅 ቀጠሮ ይያዙ', '💬 ያነጋግሩን'],
...
def main():
    ...
    try:
        ...
        # Start the bot with enhanced settings and better error handling
